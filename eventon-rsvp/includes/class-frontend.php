	...
	*/
	...
	// EVENTTOP inclusion
				...
				if( $RSVP->can_user_rsvp() ){
					...
					if($RSVP->has_space_to_rsvp()){
						...
						if(!$user_rsvp_status && !$closeRSVPbeforeX && $can_still_rsvp){
							...
							$output .=  "<span class='evors_eventtop_section evors_eventtop_rsvp evors_eventop_rsvped_data' data-eid='{$RSVP->event->ID}' data-ri='{$RSVP->ri}'data-uid='{$this_user_id}' data-lang='{$lang}'>".$TEXT. $this->get_rsvp_choices($this->opt2, $this->optRS)."</span>";
						}else{
						// user has rsvp-ed already
							$TEXT = evo_lang('You have already RSVP-ed', $lang);
							$output .="<span class='evors_eventtop_section evors_eventtop_rsvp evors_eventop_rsvped_data'>{$TEXT}: <em class='evors_rsvped_status_user'>".$this->get_rsvp_status($user_rsvp_status)."</em></span>";
						}
					}
				}
			}
		...
		// ABOVE title - sold out tag
			function eventtop_above_title($var, $object, $EVENT){
				...
				// Initial Check
				if(!$RSVP->is_rsvp_active()) return $var;
		...
		// add RSVP box to front end
			function frontend_box($object, $helpers, $EVENT){
				...
				// INITIAL VALUES
					...
					// check if RSVP is ok to show
						...
						$closeRSVPbeforeX = $EV->close_rsvp_beforex();
						$can_still_rsvp = $EV->can_rsvp();
				...
				ob_start();
					...
					echo  "<div class='evorow evcal_evdata_row bordb evcal_evrow_sm evo_metarow_rsvp".$helpers['end_row_class']."' >";
						...
						// Event Card content
						$show_eventcard_rsvp_content = apply_filters('evors_eventcard_content_show',true, $RR);	
			...
			// Event Card Content
				function _get_event_card_content($RSVP, $RR){
					...
					// there are RSVP spots remaining OR user loggedin
						if( $RSVP->has_space_to_rsvp() || $current_user_rsvped ){					
			...
			// get all the data values pertaining to event
				function event_rsvp_data($EV, $other_data=''){
					...
					$data_array = array();
					$data_array['etitle'] = get_the_title($EV->event->ID);
	...
	// SAVE new RSVP
		function _form_save_rsvp($args){
			...
			$R = new EVORS_Event($event_id);

			// add new rsvp
			if($created_rsvp_id = $R->add_new() ){

				EVO()->cal->set_cur('evcal_rs');

				$RR = new EVO_RSVP_CPT($created_rsvp_id);
				...
				// save loggedin user ID if prefill fields for loggedin enabled
					...
					if( $prefill_enabled || !empty($args['uid'])){
						$loggedinUserID = $R->get_current_userid();
						...
						$R->save_user_rsvp_status($CURRENT_user_id, $args['e_id'], $__repeat_interval, $args['rsvp']);
					}
				...
				// SYNC event's rsvp counts
					if($RR->get_rsvp_type() == 'normal') $R->sync_rsvp_count();
				...
				// PLUG
				do_action('evors_new_rsvp_saved', $created_rsvp_id, $args, $RR, $R);
