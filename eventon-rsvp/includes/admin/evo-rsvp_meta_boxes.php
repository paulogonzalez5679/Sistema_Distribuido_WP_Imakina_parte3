			...
			<?php
				...
				foreach($table_rows as $key=>$data){
					switch($data['type']){
						...
						case 'checkin_status':
							?>
							<tr><td><?php _e('Checkin to Event Status','evors');?>: </td><td><span class='rsvp_ch_st evors_trig_checkin <?php echo $RR->checkin_status();?>' data-status='<?php echo $RR->checkin_status();?>' data-nonce="<?php echo wp_create_nonce(AJDE_EVCAL_BASENAME);?>" data-rsvp_id='<?php echo $post->ID;?>'><?php echo $checkin_status;?></span>
			...
			<div id='evors_details' class='evors_details evomb_body ' <?php echo ( $evors_rsvp=='yes')? null:'style="display:none"'; ?>>		
				...
				<table width='100%' class='eventon_settings_table'>
						...
						<div id='evors_ri_cap' class='evotx_repeat_capacity' style='padding-top:15px; padding-bottom:20px;display:<?php echo evo_meta_yesno($pmv,'_manage_repeat_cap_rs','yes','','none' );?>'>
							...
							<?php
								...
								if($repeat_intervals && count($repeat_intervals)>0){
									...
									echo $count>10? "<p class='evors_repeats_showrest'>".__('Show Rest','evors')."</p>":'';
									echo "</div>";
					...
					<!-- close rsvp before X time -->
						...
						<tr>
							<td><?php _e('Close RSVP before event start (in minutes)','evors');?><?php echo $eventon->throw_guide(__('Set how many minutes before the event start time to close RSVP form. Time must be in minutes. Leave blank to not close RSVP before event time.','evors'));?></td>
					...
					<!-- show only certain additional fields -->
						<tr><td colspan='2'>
							<?php $this->html_yesno_fields('_evors_form_af_filter', 
								__('Show only certain additional fields in RSVP form (separated by commas)','evors'), 
								$pmv, 
								...
								'_evors_form_af_filter');?>						
						</td></tr>
						<tr class='_evors_form_af_filter yesnosub' style='display:<?php echo evo_check_yn($pmv,'_evors_form_af_filter')?'':'none';?>'>
					...
					<!-- additional information for rsvped -->
						<tr>
							<td colspan='2' style='padding-top:10px;'><?php _e('Additional Information only visible to loggedin RSVPed guests & in Confirmation Email','evors');?><?php echo $eventon->throw_guide(__('Information entered in here will only be visible on front-end once user has RSVPed to the event.','evors'));?>
				...
				<div class='evcal_rep evors_info_actions'>				
					...
					// lightbox content for emailing section
						...
						<div id='evors_emailing' style=''>
							<p><label><?php _e('Select Emailing Option','evors');?></label>
