		...
		function get_user_events($userid){
			...
			if($rsvps->have_posts()):					
				...
				while( $rsvps->have_posts() ): $rsvps->the_post();
					...
					$event_start_time_unix = $eRSVP->event->get_event_time();					
										
					// individual event class values
						$p_classes = array();
						$p_classes[] = ($event_start_time_unix >=$currentTime)?'':'pastevent';
					...
					$output.= "<p id='rsvp_event_{$event_id}' class='rsvpmanager_event ".(count($p_classes)>0? implode(' ', $p_classes):'')."'>" . evo_lang('RSVP ID'). ": <b>#".$_id."</b> <span class='rsvpstatus status_{$RSVP->trans_rsvp_status()}'>{$RSVP->trans_rsvp_status()}</span>
						<em class='checkin_status'>". EVORS()->frontend->get_checkin_status($checkin_status)."</em><br/>
						<em class='count'>".( $RSVP->count() )."</em>
						<em class='event_data' >
					...
					$output.= ($event_start_time_unix >=$currentTime)? 
					...
					// based on live or past event arrange rsvped events
						if($event_start_time_unix >=$currentTime){
							$content['live'][$event_start_time_unix.'.'.$event_id] = $output;
						}else{							
							$content['past'][$event_start_time_unix.'.'.$event_id] = $output;
						}					
				endwhile;
