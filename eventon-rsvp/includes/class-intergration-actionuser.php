...
class evors_actionuser{
	...
	// event manager additions
		...
		function evors_ajax_get_auem_stats(){
			...
			// set global language
			if(isset($_POST['data']) && isset($_POST['data']['lang']) ){
				evo_set_global_lang( $_POST['data']['lang'] );
			}
		...
		function each_attendee_data_row($_id, $rsvp, $text){
			...
			?>
			<li data-rsvpid='<?php echo $_id;?>'><em class='evorsadmin_rsvp' title='<?php _e('Click for more information','eventon');?>'><?php echo '#'.$_id;?></em><?php echo ' '. $rsvp['name'].' <i>('.$rsvp['email'].( $phone? ' '. evo_lang('PHONE') .':'.$phone:'').')</i>';?><span data-id='<?php echo $_id;?>' data-status='<?php echo $_status;?>' class='checkin <?php echo ($_status=='checked')? 'checked':null;?>'><?php echo $_status;?></span><span><?php echo $rsvp['count'];?></span>
				<?php 
