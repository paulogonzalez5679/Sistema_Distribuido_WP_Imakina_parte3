...
// Success message content
	function form_message($RSVP, $rsvpid, $form_type='submit', $post){
		...
		// pluggable proceed check
			$proceed = apply_filters('evors_rsvp_form_message',true, $form_type, $RSVP, $RR, $post);
	...
	<div class='rsvp_confirmation form_section' data-rsvpid='<?php echo $rsvpid;?>'>
		...
		<?php if($form_type=='submit'):?>
			<?php
				$_html_header = $front->replace_en( apply_filters('evors_form_success_msg_header', $eventon_rs->lang( 'evoRSL_x5','Successfully RSVP-ed for [event-name]'), $RSVP_cpt ) , $eventName );
			?>
		...
		<p><?php echo $eventon_rs->lang( 'evoRSL_x7','Thank You');?> 
